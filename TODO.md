...
- [x] Role-based field visibility (hide sensitive fields from non-supervisors) ✅ Done 2026-04-18. Added `canSeeSensitiveField()` and `redactPersonForViewer()` utilities. App Admin/HR see all; supervisors see direct reports (except SSN/MedicalNotes/clearance level); others see only public fields (Name/Email/Position/Org).
...
- [x] **NF-14: Integrate with Security incident lifecycle** — 💻 ✅ Done 2026-04-19. Status transitions send templated notifications. Communications tab now built in IM-14 tabbed detail modal with `CCSD_IncidentNotifications` display.
...
- [x] **NF-16: Integrate with Training module** — 💻 ✅ Done 2026-04-19. Training expiration notifications via `initNotificationChecks()`. Training approval/rejection notifications now sent via `sendNotification()` in `reviewSubmission()`.
...
- [x] **IM-14: Build incident detail modal** — 💻 ✅ Done 2026-04-19. Full tabbed layout with 7 tabs: Summary (case fields + status transition), Timeline (status history with days-in-status), Actions (table with add/complete, overdue highlighting), Communications (notification log from CCSD_IncidentNotifications + send button), Parties (from CCSD_IncidentParties, add internal/external), Attachments (SharePoint AttachmentFiles API), SOR (SOR fields + countdown, visible when status >= SOR Issued).
...
- [x] **IM-18: Build incident audit logging** — 💻 ✅ Done 2026-04-19. View events logged on detail modal open. Create/Update auto-logged by infrastructure. Status transitions logged with before-snapshot. SOR tab now rendered in IM-14; SOR field changes logged via `updateListItem` audit trail.
...
- [x] **SH-07: Build landing page / overview dashboard** — 💻 ✅ Done 2026-04-19. KPI strip, pending actions, readiness scorecard (Training Compliance %, DPMAP On Track %, IDP On File %, Present for Duty % with color-coded progress bars), and recent activity feed (last 10 team audit log entries).
- [x] **SH-08: Build team roster table** — 💻 ✅ Done 2026-04-19. Sortable roster with debounced search bar, status filter pills with counts, and per-row quick-action dropdown menus (View Profile, View Calendar, Send Notification, Team Summary).
